...
<br>3- one to be used in the remote application - the one running within the VDI Toolkit software on the client PC (eLux Thin Client, Windows/Mac personal PC, etc.)(only required in non-legacy eLux)
...
### Initialization
The `initializeHIDDevices` API is used to initialize the SDK.
...
- A `mode` key is **required**. Valid values for `mode` are `'VDI'`, `'desktop'` or '`browser`'
